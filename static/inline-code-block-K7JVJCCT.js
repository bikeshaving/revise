...
// node_modules/@b9g/revise/src/state.js
init_Buffer();
init_process();

// node_modules/@b9g/revise/src/history.js
init_Buffer();
init_process();
...
// node_modules/@b9g/revise/src/keyer.js
init_Buffer();
init_process();
var Keyer = class {
  nextKey;
...
var BG_DARK = "#0f1117";
var BG_LIGHT = "#f0f4f8";
var TEXT_DARK = "#e4e4e7";
var TEXT_LIGHT = "#1a1b26";
function getColorSchemeScript() {
  return `
...
// src/components/code-preview.ts
function generateImportMap(staticURLs) {
  const imports = {};
...
function* CodePreview({
  ...
  if (typeof window !== "undefined") {
    ...
    execute = () => {
      ...
      let code = value;
      console.log("[CodePreview] execute", { id, valueLength: value == null ? void 0 : value.length, valueFirst50: value == null ? void 0 : value.substring(0, 50) });
      try {
        const parsed = transform(value);
        code = parsed.code;
        console.log("[CodePreview] transform result", { codeLength: code == null ? void 0 : code.length, codeFirst100: code == null ? void 0 : code.substring(0, 100) });
  ...
  if (typeof window !== "undefined") {
    const onmessage = (ev) => {
      let data2 = JSON.parse(ev.data);
      if (data2.id !== id) {
        return;
      }
      if (data2.type === "executed") {
        this.refresh(() => {
          loading = false;
        });
      ...
      } else if (data2.type === "resize" && visible) {
        if (autoresize) {
          setTimeout(() => {
            this.refresh(() => {
              height = Math.min(1e3, Math.max(100, data2.height));
            });
          });
        }
      }
  ...
  let oldValue;
  let oldVisible;
  console.log("[CodePreview] before for-of, initial value length:", value == null ? void 0 : value.length);
  for ({ value, visible = true, autoresize = false } of this) {
    console.log("[CodePreview] for-of iteration", { id, valueLength: value == null ? void 0 : value.length, visible });
        ...
        showErrorModal = false;
									...
									`}>
										<input
											...
											onchange=${(ev) => {
      suppressErrors = ev.target.checked;
    }}
										/>
										Suppress this modal
									</label>
			...
			`}>
				...
				`}>
					<div hydrate="!class" class=${css`
						...
						overflow: hidden;
						${editable ? `@media (min-width: ${breakpoint}) {
							max-width: 61.8%;
