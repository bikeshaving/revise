...
// node_modules/@b9g/crank/async.js
...
async function* Suspense({ children, fallback, timeout }) {
  ...
  this.provide(SuspenseListController, void 0);
...
var SuspenseListController = /* @__PURE__ */ Symbol.for("SuspenseListController");
  ...
  const controller = {
    ...
    async scheduleChildren(ctx, suspenseItems) {
      ...
      if (revealOrder === "together") {
        ...
        const waitFor = suspenseItems.slice(0, index + 1).map((item) => item.promise);
        await Promise.all(waitFor);
      } else if (revealOrder === "backwards") {
        const waitFor = suspenseItems.slice(index + 1).map((item) => item.promise);
        await Promise.all(waitFor);
      }
    }
  };
