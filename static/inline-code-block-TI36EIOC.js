...
// ../packages/crankeditable/src/crank-editable.ts
init_Buffer();
init_process();
...
function invalidate(_this, records) {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    for (let j = 0; j < record.addedNodes.length; j++) {
      ...
      clear(addedNode, cache2);
    }
    for (let j = 0; j < record.removedNodes.length; j++) {
      clear(record.removedNodes[j], cache2);
    }
...
function diff(_this, oldValue, oldSelectionStart) {
  ...
  for (let node2 = _this, descending = true, offset2 = 0, oldIndex = 0, oldIndexRelative = 0, hasNewline = false; ; node2 = walker.currentNode) {
    if (descending) {
      ...
      if (nodeInfo === void 0) {
        cache2.set(node2, nodeInfo = new NodeInfo(offset2));
...
function findNodeOffset(_this, index) {
  ...
  for (let node22 = _this; node22 !== null; ) {
    ...
    if (index === nodeInfo.length && node22.nodeType === Node.TEXT_NODE) {
      ...
      if (node22.nodeType === Node.ELEMENT_NODE && node22.hasAttribute("data-content")) {
        return nodeOffsetFromChild(node22, index > 0);
      }
      const firstChild = walker.firstChild();
      if (firstChild === null) {
        const offset2 = node22.nodeType === Node.TEXT_NODE ? index : index > 0 ? 1 : 0;
