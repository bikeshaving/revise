...
init_process();
var NONE2 = 0;
var CAPTURING_PHASE2 = 1;
var AT_TARGET2 = 2;
var BUBBLING_PHASE2 = 3;
function isEventTarget2(value) {
  return value != null && typeof value.addEventListener === "function" && typeof value.removeEventListener === "function" && typeof value.dispatchEvent === "function";
}
function setEventProperty2(ev, key, value) {
  Object.defineProperty(ev, key, { value, writable: false, configurable: true });
}
function isListenerOrListenerObject2(value) {
  return typeof value === "function" || value !== null && typeof value === "object" && typeof value.handleEvent === "function";
}
function normalizeListenerOptions2(options) {
  if (typeof options === "boolean") {
    return { capture: options };
  } else if (options == null) {
    return {};
...
var _parent2 = /* @__PURE__ */ Symbol.for("CustomEventTarget.parent");
var _listeners2 = /* @__PURE__ */ Symbol.for("CustomEventTarget.listeners");
var _delegates2 = /* @__PURE__ */ Symbol.for("CustomEventTarget.delegates");
var _dispatchEventOnSelf2 = /* @__PURE__ */ Symbol.for("CustomEventTarget.dispatchSelf");
var CustomEventTarget2 = class {
  constructor(parent = null) {
    this[_parent2] = parent;
    this[_listeners2] = [];
    this[_delegates2] = /* @__PURE__ */ new Set();
  }
  addEventListener(type, listener, options) {
    if (!isListenerOrListenerObject2(listener)) {
      return;
    }
    const listeners = this[_listeners2];
    options = normalizeListenerOptions2(options);
    let callback;
    if (typeof listener === "function") {
      callback = listener;
    } else {
    ...
    listeners.push(record);
    for (const delegate of this[_delegates2]) {
      delegate.addEventListener(type, record.callback, record.options);
    }
  }
  removeEventListener(type, listener, options) {
    const listeners = this[_listeners2];
    if (listeners == null || !isListenerOrListenerObject2(listener)) {
      return;
    }
    const options1 = normalizeListenerOptions2(options);
    const i = listeners.findIndex((record2) => record2.type === type && record2.listener === listener && !record2.options.capture === !options1.capture);
    if (i === -1) {
      return;
    }
  ...
  dispatchEvent(ev) {
    const path = [];
    for (let parent = this[_parent2]; parent; parent = parent[_parent2]) {
      path.push(parent);
    }
    let cancelBubble = false;
    let immediateCancelBubble = false;
    const stopPropagation = ev.stopPropagation;
    setEventProperty2(ev, "stopPropagation", () => {
      cancelBubble = true;
      return stopPropagation.call(ev);
    });
    const stopImmediatePropagation = ev.stopImmediatePropagation;
    setEventProperty2(ev, "stopImmediatePropagation", () => {
      immediateCancelBubble = true;
      return stopImmediatePropagation.call(ev);
    });
    setEventProperty2(ev, "target", this);
    try {
      setEventProperty2(ev, "eventPhase", CAPTURING_PHASE2);
      for (let i = path.length - 1; i >= 0; i--) {
        const target = path[i];
        const listeners = target[_listeners2];
        setEventProperty2(ev, "currentTarget", target);
        for (let i2 = 0; i2 < listeners.length; i2++) {
          const record = listeners[i2];
          if (record.type === ev.type && record.options.capture) {
            try {
      ...
      {
        setEventProperty2(ev, "eventPhase", AT_TARGET2);
        setEventProperty2(ev, "currentTarget", this);
        this[_dispatchEventOnSelf2](ev);
        if (immediateCancelBubble) {
          return true;
        }
        const listeners = this[_listeners2];
      ...
      if (ev.bubbles) {
        setEventProperty2(ev, "eventPhase", BUBBLING_PHASE2);
        for (let i = 0; i < path.length; i++) {
          const target = path[i];
          setEventProperty2(ev, "currentTarget", target);
          const listeners = target[_listeners2];
      ...
      setEventProperty2(ev, "eventPhase", NONE2);
      setEventProperty2(ev, "currentTarget", null);
      return !ev.defaultPrevented;
    }
  }
  [_dispatchEventOnSelf2](_ev) {
  }
};
CustomEventTarget2.dispatchEventOnSelf = _dispatchEventOnSelf2;
function addEventTargetDelegates2(target, delegates, include = (target1) => target === target1) {
  const delegates1 = delegates.filter(isEventTarget2);
  for (let target1 = target; target1 && include(target1); target1 = target1[_parent2]) {
    for (let i = 0; i < delegates1.length; i++) {
      const delegate = delegates1[i];
      if (target1[_delegates2].has(delegate)) {
        continue;
      }
      target1[_delegates2].add(delegate);
      for (const record of target1[_listeners2]) {
        delegate.addEventListener(record.type, record.callback, record.options);
      }
    }
  }
}
function removeEventTargetDelegates2(target, delegates, include = (target1) => target === target1) {
  const delegates1 = delegates.filter(isEventTarget2);
  for (let target1 = target; target1 && include(target1); target1 = target1[_parent2]) {
    for (let i = 0; i < delegates1.length; i++) {
      const delegate = delegates1[i];
      ...
      target1[_delegates2].delete(delegate);
      for (const record of target1[_listeners2]) {
        delegate.removeEventListener(record.type, record.callback, record.options);
      }
    }
...
function clearEventListeners2(target) {
  const listeners = target[_listeners2];
  const delegates = target[_delegates2];
  for (let i = 0; i < listeners.length; i++) {
    const record = listeners[i];
    for (const delegate of delegates) {
      delegate.removeEventListener(record.type, record.callback, record.options);
...
function unwrap2(arr) {
  return arr.length === 0 ? void 0 : arr.length === 1 ? arr[0] : arr;
}
function arrayify2(value) {
  return value == null ? [] : Array.isArray(value) ? value : typeof value === "string" || typeof value[Symbol.iterator] !== "function" ? [value] : [...value];
}
function isIteratorLike2(value) {
  return value != null && typeof value.next === "function";
}
function isPromiseLike2(value) {
  return value != null && typeof value.then === "function";
}
function createRaceRecord2(contender) {
  const deferreds = /* @__PURE__ */ new Set();
  const record = { deferreds, settled: false };
  Promise.resolve(contender).then((value) => {
    for (const { resolve } of deferreds) {
...
function safeRace2(contenders) {
  let deferred;
  const result = new Promise((resolve, reject) => {
    deferred = { resolve, reject };
    for (const contender of contenders) {
      if (!isPromiseLike2(contender)) {
        Promise.resolve(contender).then(resolve, reject);
        continue;
      }
      let record = wm2.get(contender);
      if (record === void 0) {
        record = createRaceRecord2(contender);
        record.deferreds.add(deferred);
        wm2.set(contender, record);
      } else if (record.settled) {
        Promise.resolve(contender).then(resolve, reject);
      } else {
        record.deferreds.add(deferred);
  ...
  return result.finally(() => {
    for (const contender of contenders) {
      if (isPromiseLike2(contender)) {
        const record = wm2.get(contender);
        if (record) {
          record.deferreds.delete(deferred);
        }
      }
...
var Fragment2 = "";
var Portal2 = /* @__PURE__ */ Symbol.for("crank.Portal");
var Copy2 = /* @__PURE__ */ Symbol.for("crank.Copy");
var Text2 = /* @__PURE__ */ Symbol.for("crank.Text");
var Raw2 = /* @__PURE__ */ Symbol.for("crank.Raw");
var ElementSymbol2 = /* @__PURE__ */ Symbol.for("crank.Element");
var Element3 = class {
  constructor(tag, props) {
    this.tag = tag;
    this.props = props;
...
Element3.prototype.$$typeof = ElementSymbol2;
function isElement2(value) {
  return value != null && value.$$typeof === ElementSymbol2;
}
var DEPRECATED_PROP_PREFIXES2 = ["crank-", "c-", "$"];
var DEPRECATED_SPECIAL_PROP_BASES2 = ["key", "ref", "static", "copy"];
function createElement2(tag, props, ...children) {
  if (props == null) {
    props = {};
  }
  if ("static" in props) {
  ...
  for (let i = 0; i < DEPRECATED_PROP_PREFIXES2.length; i++) {
    const propPrefix = DEPRECATED_PROP_PREFIXES2[i];
    for (let j = 0; j < DEPRECATED_SPECIAL_PROP_BASES2.length; j++) {
      const propBase = DEPRECATED_SPECIAL_PROP_BASES2[j];
      const deprecatedPropName = propPrefix + propBase;
      if (deprecatedPropName in props) {
        const targetPropBase = propBase === "static" ? "copy" : propBase;
        console.error(`The \`${deprecatedPropName}\` prop is deprecated. Use \`${targetPropBase}\` instead.`);
  ...
  return new Element3(tag, props);
}
function cloneElement2(el) {
  if (!isElement2(el)) {
    throw new TypeError(`Cannot clone non-element: ${String(el)}`);
  }
  return new Element3(el.tag, { ...el.props });
}
function narrow2(value) {
  if (typeof value === "boolean" || value == null) {
    return;
  } else if (typeof value === "string" || isElement2(value)) {
    return value;
  } else if (typeof value[Symbol.iterator] === "function") {
    return createElement2(Fragment2, null, value);
  }
  return value.toString();
}
var DidDiff2 = 1 << 0;
...
var IsCopied2 = 1 << 2;
var IsUpdating2 = 1 << 3;
var IsExecuting2 = 1 << 4;
var IsRefreshing2 = 1 << 5;
var IsScheduling2 = 1 << 6;
var IsSchedulingFallback2 = 1 << 7;
var IsUnmounted2 = 1 << 8;
var IsErrored2 = 1 << 9;
var IsResurrecting2 = 1 << 10;
var IsSyncGen2 = 1 << 11;
var IsAsyncGen2 = 1 << 12;
var IsInForOfLoop2 = 1 << 13;
var IsInForAwaitOfLoop2 = 1 << 14;
var NeedsToYield2 = 1 << 15;
var PropsAvailable2 = 1 << 16;
var IsSchedulingRefresh2 = 1 << 17;
function getFlag2(ret, flag) {
  return !!(ret.f & flag);
}
function setFlag2(ret, flag, value = true) {
  if (value) {
    ret.f |= flag;
  } else {
    ret.f &= ~flag;
...
function cloneRetainer2(ret) {
  const clone = new Retainer2(ret.el);
  clone.f = ret.f;
  clone.ctx = ret.ctx;
  clone.children = ret.children;
  clone.fallback = ret.fallback;
...
function getValue2(ret, isNested = false, index) {
  if (getFlag2(ret, IsScheduling2) && isNested) {
    return ret.fallback ? getValue2(ret.fallback, isNested, index) : void 0;
  } else if (ret.fallback && !getFlag2(ret, DidDiff2)) {
    return ret.fallback ? getValue2(ret.fallback, isNested, index) : ret.fallback;
  } else if (ret.el.tag === Portal2) {
    return;
  } else if (ret.el.tag === Fragment2 || typeof ret.el.tag === "function") {
    if (index != null && ret.ctx) {
      ret.ctx.index = index;
    }
    return unwrap2(getChildValues2(ret, index));
  }
  return ret.value;
}
function getChildValues2(ret, startIndex) {
  const values = [];
  const lingerers = ret.lingerers;
  const rawChildren = ret.children;
  const isChildrenArray = Array.isArray(rawChildren);
  ...
  for (let i = 0; i < childrenLength; i++) {
    if (lingerers != null && lingerers[i] != null) {
      const rets = lingerers[i];
      for (const ret2 of rets) {
        const value = getValue2(ret2, true, currentIndex);
        if (Array.isArray(value)) {
          for (let j = 0; j < value.length; j++) {
            values.push(value[j]);
          }
    ...
    const child = isChildrenArray ? rawChildren[i] : rawChildren;
    if (child) {
      const value = getValue2(child, true, currentIndex);
      if (Array.isArray(value)) {
        for (let j = 0; j < value.length; j++) {
          values.push(value[j]);
        }
...
function stripSpecialProps2(props) {
  let _;
  let result;
  ({ key: _, ref: _, copy: _, hydrate: _, children: _, ...result } = props);
  return result;
}
function diffChild2(adapter2, root, host, ctx, scope, parent, newChildren) {
  let child = narrow2(newChildren);
  let ret = parent.children;
  let graveyard;
  let diff2;
  if (typeof child === "object") {
    ...
    if (child.tag === Copy2) {
      childCopied = true;
    } else if (typeof ret === "object" && ret.el === child && getFlag2(ret, DidCommit2)) {
      childCopied = true;
    } else {
      if (ret && ret.el.tag === child.tag) {
        ret.el = child;
        ...
        for (let predecessor = ret, candidate = ret.fallback; candidate; predecessor = candidate, candidate = candidate.fallback) {
          if (candidate.el.tag === child.tag) {
            const clone = cloneRetainer2(candidate);
            setFlag2(clone, IsResurrecting2);
            predecessor.fallback = clone;
            const fallback = ret;
            ret = candidate;
            ret.el = child;
            ret.fallback = fallback;
            setFlag2(ret, DidDiff2, false);
            candidateFound = true;
            break;
          }
        }
        if (!candidateFound) {
          const fallback = ret;
          ret = new Retainer2(child);
          ret.fallback = fallback;
        }
      } else {
        ret = new Retainer2(child);
      }
      if (childCopied && getFlag2(ret, DidCommit2)) ;
      else if (child.tag === Raw2 || child.tag === Text2) ;
      else if (child.tag === Fragment2) {
        diff2 = diffChildren2(adapter2, root, host, ctx, scope, ret, ret.el.props.children);
      } else if (typeof child.tag === "function") {
        diff2 = diffComponent2(adapter2, root, host, ctx, scope, ret);
      } else {
        diff2 = diffHost2(adapter2, root, ctx, scope, ret);
      }
    }
    if (typeof ret === "object") {
      if (childCopied) {
        setFlag2(ret, IsCopied2);
        diff2 = getInflightDiff2(ret);
      } else {
        setFlag2(ret, IsCopied2, false);
      }
    }
  } else if (typeof child === "string") {
    if (typeof ret === "object" && ret.el.tag === Text2) {
      ret.el.props.value = child;
    } else {
      if (typeof ret === "object") {
        (graveyard = graveyard || []).push(ret);
      }
      ret = new Retainer2(createElement2(Text2, { value: child }));
    }
  } else {
    if (typeof ret === "object") {
      (graveyard = graveyard || []).push(ret);
  ...
  parent.children = ret;
  if (isPromiseLike2(diff2)) {
    const diff1 = diff2.finally(() => {
      setFlag2(parent, DidDiff2);
      if (graveyard) {
        if (parent.graveyard) {
          for (let i = 0; i < graveyard.length; i++) {
            parent.graveyard.push(graveyard[i]);
    ...
    let onNextDiffs;
    const diff22 = parent.pendingDiff = safeRace2([
      diff1,
      new Promise((resolve) => onNextDiffs = resolve)
    ]);
    if (parent.onNextDiff) {
...
function diffChildren2(adapter2, root, host, ctx, scope, parent, newChildren) {
  if (!Array.isArray(newChildren) && (typeof newChildren !== "object" || newChildren === null || typeof newChildren[Symbol.iterator] !== "function") && !Array.isArray(parent.children)) {
    return diffChild2(adapter2, root, host, ctx, scope, parent, newChildren);
  }
  const oldRetained = wrap2(parent.children);
  const newRetained = [];
  const newChildren1 = arrayify2(newChildren);
  const diffs = [];
  let childrenByKey;
  let seenKeys;
  let isAsync = false;
  ...
  for (let ni = 0, newLength = newChildren1.length; ni < newLength; ni++) {
    let ret = oi >= oldLength ? void 0 : oldRetained[oi];
    let child = narrow2(newChildren1[ni]);
    {
      let oldKey = typeof ret === "object" ? ret.el.props.key : void 0;
      let newKey = typeof child === "object" ? child.props.key : void 0;
      if (newKey !== void 0 && seenKeys && seenKeys.has(newKey)) {
        console.error(`Duplicate key found in <${getTagName2(parent.el.tag)}>`, newKey);
        child = cloneElement2(child);
        newKey = child.props.key = void 0;
      }
      if (oldKey === newKey) {
        if (childrenByKey !== void 0 && newKey !== void 0) {
        ...
        childrenByKey = childrenByKey || createChildrenByKey2(oldRetained, oi);
        if (newKey === void 0) {
          while (ret !== void 0 && oldKey !== void 0) {
            oi++;
            ret = oldRetained[oi];
  ...
  parent.children = unwrap2(newRetained);
  if (isAsync) {
    const diffs1 = Promise.all(diffs).then(() => void 0).finally(() => {
      setFlag2(parent, DidDiff2);
      if (graveyard) {
    ...
    let onNextDiffs;
    const diffs2 = parent.pendingDiff = safeRace2([
      diffs1,
      new Promise((resolve) => onNextDiffs = resolve)
    ]);
    if (parent.onNextDiff) {
...
function createChildrenByKey2(children, offset) {
  const childrenByKey = /* @__PURE__ */ new Map();
  for (let i = offset; i < children.length; i++) {
    const child = children[i];
    if (typeof child === "object" && typeof child.el.props.key !== "undefined") {
...
function diffHost2(adapter2, root, ctx, scope, ret) {
  const el = ret.el;
  const tag = el.tag;
  if (el.tag === Portal2) {
    root = ret.value = el.props.root;
  }
  if (getFlag2(ret, DidCommit2)) {
    scope = ret.scope;
  } else {
    scope = ret.scope = adapter2.scope({
      tag,
      tagName: getTagName2(tag),
      props: el.props,
      scope,
      root
    });
  }
  return diffChildren2(adapter2, root, ret, ctx, scope, ret, ret.el.props.children);
}
function commit2(adapter2, host, ret, ctx, scope, root, index, schedulePromises, hydrationNodes) {
  if (getFlag2(ret, IsCopied2) && getFlag2(ret, DidCommit2)) {
    return getValue2(ret);
  }
  const el = ret.el;
  const tag = el.tag;
  if (typeof tag === "function" || tag === Fragment2 || tag === Portal2 || tag === Raw2 || tag === Text2) {
    if (typeof el.props.copy === "string") {
      console.error(`String copy prop ignored for <${getTagName2(tag)}>. Use booleans instead.`);
    }
    if (typeof el.props.hydrate === "string") {
      console.error(`String hydrate prop ignored for <${getTagName2(tag)}>. Use booleans instead.`);
    }
  }
  let value;
  let skippedHydrationNodes;
  ...
  if (typeof tag === "function") {
    ret.ctx.index = index;
    value = commitComponent2(ret.ctx, schedulePromises, hydrationNodes);
  } else {
    if (tag === Fragment2) {
      value = commitChildren2(adapter2, host, ctx, scope, root, ret, index, schedulePromises, hydrationNodes);
    } else if (tag === Text2) {
      value = commitText2(adapter2, ret, el, scope, hydrationNodes, root);
    } else if (tag === Raw2) {
      value = commitRaw2(adapter2, host, ret, scope, hydrationNodes, root);
    } else {
      value = commitHost2(adapter2, ret, ctx, root, schedulePromises, hydrationNodes);
    }
    if (ret.fallback) {
      unmount2(adapter2, host, ctx, root, ret.fallback, false);
      ret.fallback = void 0;
    }
  }
  if (skippedHydrationNodes) {
  ...
  if (!getFlag2(ret, DidCommit2)) {
    setFlag2(ret, DidCommit2);
    if (typeof tag !== "function" && tag !== Fragment2 && tag !== Portal2 && typeof el.props.ref === "function") {
      el.props.ref(adapter2.read(value));
    }
  }
  return value;
}
function commitChildren2(adapter2, host, ctx, scope, root, parent, index, schedulePromises, hydrationNodes) {
  let values = [];
  const rawChildren = parent.children;
  const isChildrenArray = Array.isArray(rawChildren);
  const childrenLength = rawChildren === void 0 ? 0 : isChildrenArray ? rawChildren.length : 1;
  for (let i = 0; i < childrenLength; i++) {
    ...
    let schedulePromises1;
    let isSchedulingFallback = false;
    while (child && (!getFlag2(child, DidDiff2) && child.fallback || getFlag2(child, IsScheduling2))) {
      if (getFlag2(child, IsScheduling2) && child.ctx.schedule) {
        (schedulePromises1 = schedulePromises1 || []).push(child.ctx.schedule.promise);
        isSchedulingFallback = true;
      }
      if (!getFlag2(child, DidDiff2) && getFlag2(child, DidCommit2)) {
        for (const node of getChildValues2(child)) {
          adapter2.remove({
            node,
            parentNode: host.value,
            isNested: false,
      ...
      child = child.fallback;
      if (schedulePromises1 && isSchedulingFallback && child) {
        if (!getFlag2(child, DidDiff2)) {
          const inflightDiff = getInflightDiff2(child);
          schedulePromises1.push(inflightDiff);
        } else {
          schedulePromises1 = void 0;
        }
        if (getFlag2(child, IsSchedulingFallback2)) {
          isSchedulingFallback = true;
        } else {
          setFlag2(child, IsSchedulingFallback2, true);
          isSchedulingFallback = false;
        }
      }
    }
    if (schedulePromises1 && schedulePromises1.length > 1) {
      schedulePromises.push(safeRace2(schedulePromises1));
    }
    if (child) {
      const value = commit2(adapter2, host, child, ctx, scope, root, index, schedulePromises, hydrationNodes);
      if (Array.isArray(value)) {
        for (let j = 0; j < value.length; j++) {
          values.push(value[j]);
        }
  ...
  if (parent.graveyard) {
    for (let i = 0; i < parent.graveyard.length; i++) {
      const child = parent.graveyard[i];
      unmount2(adapter2, host, ctx, root, child, false);
    }
    parent.graveyard = void 0;
  }
  if (parent.lingerers) {
    values = getChildValues2(parent);
  }
  return values;
}
function commitText2(adapter2, ret, el, scope, hydrationNodes, root) {
  const value = adapter2.text({
    value: el.props.value,
    scope,
    oldNode: ret.value,
...
function commitRaw2(adapter2, host, ret, scope, hydrationNodes, root) {
  if (!ret.oldProps || ret.oldProps.value !== ret.el.props.value) {
    const oldNodes = wrap2(ret.value);
    for (let i = 0; i < oldNodes.length; i++) {
      const oldNode = oldNodes[i];
      adapter2.remove({
        node: oldNode,
  ...
  ret.oldProps = stripSpecialProps2(ret.el.props);
  return ret.value;
}
function commitHost2(adapter2, ret, ctx, root, schedulePromises, hydrationNodes) {
  if (getFlag2(ret, IsCopied2) && getFlag2(ret, DidCommit2)) {
    return getValue2(ret);
  }
  const tag = ret.el.tag;
  const props = stripSpecialProps2(ret.el.props);
  const oldProps = ret.oldProps;
  let node = ret.value;
  let copyProps;
  let copyChildren = false;
  if (oldProps) {
    for (const propName in props) {
      if (props[propName] === Copy2) {
        props[propName] = oldProps[propName];
        (copyProps = copyProps || /* @__PURE__ */ new Set()).add(propName);
      }
    }
    if (typeof ret.el.props.copy === "string") {
      const copyMetaProp = new MetaProp2("copy", ret.el.props.copy);
      if (copyMetaProp.include) {
        for (const propName of copyMetaProp.props) {
          if (propName in oldProps) {
            props[propName] = oldProps[propName];
  ...
  if (!getFlag2(ret, DidCommit2)) {
    if (tag === Portal2) {
      ...
      if (!node && hydrationNodes) {
        const nextChild = hydrationNodes.shift();
        if (typeof ret.el.props.hydrate === "string") {
          hydrationMetaProp = new MetaProp2("hydration", ret.el.props.hydrate);
          if (hydrationMetaProp.include) {
            quietProps = new Set(Object.keys(props));
            for (const propName of hydrationMetaProp.props) {
              quietProps.delete(propName);
  ...
  if (!copyChildren) {
    const children = commitChildren2(adapter2, ret, ctx, scope, tag === Portal2 ? node : root, ret, 0, schedulePromises, hydrationMetaProp && !hydrationMetaProp.includes("children") ? void 0 : childHydrationNodes);
    adapter2.arrange({
      tag,
      tagName: getTagName2(tag),
      node,
...
function contextContains2(parent, child) {
  for (let current = child; current !== void 0; current = current.parent) {
    if (current === parent) {
      return true;
    }
...
var ANONYMOUS_ROOT2 = {};
function flush2(adapter2, root, initiator) {
  if (root != null) {
    adapter2.finalize(root);
  }
  if (typeof root !== "object" || root === null) {
    root = ANONYMOUS_ROOT2;
  }
  const afterMap = afterMapByRoot2.get(root);
  if (afterMap) {
    const afterMap1 = /* @__PURE__ */ new Map();
    for (const [ctx, callbacks] of afterMap) {
      if (getFlag2(ctx.ret, IsScheduling2) || initiator && !contextContains2(initiator, ctx)) {
        afterMap.delete(ctx);
        afterMap1.set(ctx, callbacks);
      }
    }
    if (afterMap1.size) {
      afterMapByRoot2.set(root, afterMap1);
    } else {
      afterMapByRoot2.delete(root);
    }
    for (const [ctx, callbacks] of afterMap) {
      const value = adapter2.read(getValue2(ctx.ret));
      for (const callback of callbacks) {
        callback(value);
      }
    }
...
function unmount2(adapter2, host, ctx, root, ret, isNested) {
  if (ret.fallback) {
    unmount2(adapter2, host, ctx, root, ret.fallback, isNested);
    ret.fallback = void 0;
  }
  if (getFlag2(ret, IsResurrecting2)) {
    return;
  }
  if (ret.lingerers) {
    for (let i = 0; i < ret.lingerers.length; i++) {
      ...
      if (lingerers) {
        for (const lingerer of lingerers) {
          unmount2(adapter2, host, ctx, root, lingerer, isNested);
        }
      }
    }
    ret.lingerers = void 0;
  ...
  if (typeof ret.el.tag === "function") {
    unmountComponent2(ret.ctx, isNested);
  } else if (ret.el.tag === Fragment2) {
    unmountChildren2(adapter2, host, ctx, root, ret, isNested);
  } else if (ret.el.tag === Portal2) {
    unmountChildren2(adapter2, ret, ctx, ret.value, ret, false);
    if (ret.value != null) {
      adapter2.finalize(ret.value);
    }
  } else {
    unmountChildren2(adapter2, ret, ctx, root, ret, true);
    if (getFlag2(ret, DidCommit2)) {
      if (ctx) {
        removeEventTargetDelegates2(ctx.ctx, [ret.value], (ctx1) => ctx1[_ContextState2].host === host);
      }
      adapter2.remove({
        node: ret.value,
        parentNode: host.value,
...
function unmountChildren2(adapter2, host, ctx, root, ret, isNested) {
  if (ret.graveyard) {
    for (let i = 0; i < ret.graveyard.length; i++) {
      const child = ret.graveyard[i];
      unmount2(adapter2, host, ctx, root, child, isNested);
    }
    ret.graveyard = void 0;
  }
  const rawChildren = ret.children;
  if (Array.isArray(rawChildren)) {
    ...
    unmount2(adapter2, host, ctx, root, rawChildren, isNested);
  }
}
var provisionMaps2 = /* @__PURE__ */ new WeakMap();
var scheduleMap2 = /* @__PURE__ */ new WeakMap();
var cleanupMap2 = /* @__PURE__ */ new WeakMap();
var afterMapByRoot2 = /* @__PURE__ */ new WeakMap();
var ContextState2 = class {
  constructor(adapter2, root, host, parent, scope, ret) {
    this.adapter = adapter2;
    this.root = root;
    this.host = host;
    this.parent = parent;
    this.ctx = new Context2(this);
    this.scope = scope;
    this.ret = ret;
    this.iterator = void 0;
    this.inflight = void 0;
...
var _ContextState2 = /* @__PURE__ */ Symbol.for("crank.ContextState");
var Context2 = class extends CustomEventTarget2 {
  // TODO: If we could make the constructor function take a nicer value, it
  // would be useful for testing purposes.
  constructor(state) {
    super(state.parent ? state.parent.ctx : null);
    this[_ContextState2] = state;
  }
  /**
   * The current props of the associated element.
   */
  get props() {
    return this[_ContextState2].ret.el.props;
  }
  /**
   * The current value of the associated element.
   *
  ...
  get value() {
    console.warn("Context.value is deprecated.");
    return this[_ContextState2].adapter.read(getValue2(this[_ContextState2].ret));
  }
  get isExecuting() {
    return getFlag2(this[_ContextState2].ret, IsExecuting2);
  }
  get isUnmounted() {
    return getFlag2(this[_ContextState2].ret, IsUnmounted2);
  }
  *[Symbol.iterator]() {
    const ctx = this[_ContextState2];
    setFlag2(ctx.ret, IsInForOfLoop2);
    try {
      while (!getFlag2(ctx.ret, IsUnmounted2) && !getFlag2(ctx.ret, IsErrored2)) {
        if (getFlag2(ctx.ret, NeedsToYield2)) {
          throw new Error(`<${getTagName2(ctx.ret.el.tag)}> context iterated twice without a yield`);
        } else {
          setFlag2(ctx.ret, NeedsToYield2);
        }
        yield ctx.ret.el.props;
      }
    } finally {
      setFlag2(ctx.ret, IsInForOfLoop2, false);
    }
  }
  async *[Symbol.asyncIterator]() {
    const ctx = this[_ContextState2];
    setFlag2(ctx.ret, IsInForAwaitOfLoop2);
    try {
      while (!getFlag2(ctx.ret, IsUnmounted2) && !getFlag2(ctx.ret, IsErrored2)) {
        if (getFlag2(ctx.ret, NeedsToYield2)) {
          throw new Error(`<${getTagName2(ctx.ret.el.tag)}> context iterated twice without a yield`);
        ...
        if (getFlag2(ctx.ret, PropsAvailable2)) {
          setFlag2(ctx.ret, PropsAvailable2, false);
          yield ctx.ret.el.props;
        } else {
          const props = await new Promise((resolve) => ctx.onPropsProvided = resolve);
          if (getFlag2(ctx.ret, IsUnmounted2) || getFlag2(ctx.ret, IsErrored2)) {
            break;
          }
          yield props;
        }
      ...
      setFlag2(ctx.ret, IsInForAwaitOfLoop2, false);
      if (ctx.onPropsRequested) {
        ctx.onPropsRequested();
        ctx.onPropsRequested = void 0;
      }
  ...
  refresh(callback) {
    const ctx = this[_ContextState2];
    if (getFlag2(ctx.ret, IsUnmounted2)) {
      console.error(`Component <${getTagName2(ctx.ret.el.tag)}> is unmounted. Check the isUnmounted property if necessary.`);
      return ctx.adapter.read(getValue2(ctx.ret));
    } else if (getFlag2(ctx.ret, IsExecuting2)) {
      console.error(`Component <${getTagName2(ctx.ret.el.tag)}> is already executing Check the isExecuting property if necessary.`);
      return ctx.adapter.read(getValue2(ctx.ret));
    }
    if (callback) {
      const result = callback();
    ...
    if (getFlag2(ctx.ret, IsScheduling2)) {
      setFlag2(ctx.ret, IsSchedulingRefresh2);
    }
    const commitLabel = "commit (" + getTagName2(ctx.ret.el.tag) + ")";
    let diff2;
    const schedulePromises = [];
    try {
      setFlag2(ctx.ret, IsRefreshing2);
      diff2 = enqueueComponent2(ctx);
      if (isPromiseLike2(diff2)) {
        return diff2.then(() => {
          markStart(commitLabel);
          const value = commitComponent2(ctx, schedulePromises);
          ...
          const diff3 = propagateError2(ctx, err, schedulePromises);
          if (diff3) {
            return diff3.then(() => {
              if (schedulePromises.length) {
                return Promise.all(schedulePromises).then(() => {
          ...
          return ctx.adapter.read(getValue2(ctx.ret));
        }).finally(() => setFlag2(ctx.ret, IsRefreshing2, false));
      }
      markStart(commitLabel);
      const result = ctx.adapter.read(commitComponent2(ctx, schedulePromises));
      measureMark(commitLabel);
      if (schedulePromises.length) {
        return Promise.all(schedulePromises).then(() => {
          return ctx.adapter.read(getValue2(ctx.ret));
      ...
      if (diff3) {
        ...
        }).then(() => ctx.adapter.read(getValue2(ctx.ret)));
      }
      if (schedulePromises.length) {
        return Promise.all(schedulePromises).then(() => {
          return ctx.adapter.read(getValue2(ctx.ret));
      ...
      if (!isPromiseLike2(diff2)) {
        setFlag2(ctx.ret, IsRefreshing2, false);
      }
    }
  }
  schedule(callback) {
    ...
    const ctx = this[_ContextState2];
    let callbacks = scheduleMap2.get(ctx);
    if (!callbacks) {
      callbacks = /* @__PURE__ */ new Set();
      scheduleMap2.set(ctx, callbacks);
    }
    callbacks.add(callback);
  }
  after(callback) {
    ...
    const ctx = this[_ContextState2];
    const root = ctx.root || ANONYMOUS_ROOT2;
    let afterMap = afterMapByRoot2.get(root);
    if (!afterMap) {
      afterMap = /* @__PURE__ */ new Map();
      afterMapByRoot2.set(root, afterMap);
    }
    let callbacks = afterMap.get(ctx);
    if (!callbacks) {
      callbacks = /* @__PURE__ */ new Set();
  ...
  cleanup(callback) {
    ...
    const ctx = this[_ContextState2];
    if (getFlag2(ctx.ret, IsUnmounted2)) {
      const value = ctx.adapter.read(getValue2(ctx.ret));
      callback(value);
      return;
    }
    let callbacks = cleanupMap2.get(ctx);
    if (!callbacks) {
      callbacks = /* @__PURE__ */ new Set();
      cleanupMap2.set(ctx, callbacks);
    }
    callbacks.add(callback);
  }
  consume(key) {
    for (let ctx = this[_ContextState2].parent; ctx !== void 0; ctx = ctx.parent) {
      const provisions = provisionMaps2.get(ctx);
      if (provisions && provisions.has(key)) {
        return provisions.get(key);
      }
    }
  ...
  provide(key, value) {
    const ctx = this[_ContextState2];
    let provisions = provisionMaps2.get(ctx);
    if (!provisions) {
      provisions = /* @__PURE__ */ new Map();
      provisionMaps2.set(ctx, provisions);
    }
    provisions.set(key, value);
  }
  [CustomEventTarget2.dispatchEventOnSelf](ev) {
    const ctx = this[_ContextState2];
    let propCallback = ctx.ret.el.props["on" + ev.type];
    if (typeof propCallback === "function") {
      propCallback(ev);
...
function diffComponent2(adapter2, root, host, parent, scope, ret) {
  let ctx;
  if (ret.ctx) {
    ctx = ret.ctx;
    if (getFlag2(ctx.ret, IsExecuting2)) {
      console.error(`Component <${getTagName2(ctx.ret.el.tag)}> is already executing`);
      return;
    } else if (ctx.schedule) {
      return ctx.schedule.promise.then(() => {
        return diffComponent2(adapter2, root, host, parent, scope, ret);
      });
    }
  } else {
    ctx = ret.ctx = new ContextState2(adapter2, root, host, parent, scope, ret);
  }
  setFlag2(ctx.ret, IsUpdating2);
  return enqueueComponent2(ctx);
}
function diffComponentChildren2(ctx, children, isYield) {
  if (getFlag2(ctx.ret, IsUnmounted2) || getFlag2(ctx.ret, IsErrored2)) {
    return;
  } else if (children === void 0) {
    console.error(`Component <${getTagName2(ctx.ret.el.tag)}> has ${isYield ? "yielded" : "returned"} undefined. If this was intentional, ${isYield ? "yield" : "return"} null instead.`);
  }
  let diff2;
  try {
    setFlag2(ctx.ret, IsExecuting2);
    diff2 = diffChildren2(ctx.adapter, ctx.root, ctx.host, ctx, ctx.scope, ctx.ret, narrow2(children));
    if (diff2) {
      diff2 = diff2.catch((err) => handleChildError2(ctx, err));
    }
  } catch (err) {
    diff2 = handleChildError2(ctx, err);
  } finally {
    setFlag2(ctx.ret, IsExecuting2, false);
  }
  return diff2;
}
function enqueueComponent2(ctx) {
  if (!ctx.inflight) {
    const [block, diff2] = runComponent2(ctx);
    if (block) {
      ctx.inflight = [block.finally(() => advanceComponent2(ctx)), diff2];
    }
    return diff2;
  } else if (!ctx.enqueued) {
    let resolve;
    ctx.enqueued = [
      new Promise((resolve1) => resolve = resolve1).finally(() => advanceComponent2(ctx)),
      ctx.inflight[0].finally(() => {
        const [block, diff2] = runComponent2(ctx);
        resolve(block);
        return diff2;
...
function advanceComponent2(ctx) {
  ctx.inflight = ctx.enqueued;
  ctx.enqueued = void 0;
}
function runComponent2(ctx) {
  if (getFlag2(ctx.ret, IsUnmounted2)) {
    return [void 0, void 0];
  }
  const ret = ctx.ret;
  const initial = !ctx.iterator;
  const tagName = getTagName2(ret.el.tag);
  if (initial) {
    setFlag2(ctx.ret, IsExecuting2);
    clearEventListeners2(ctx.ctx);
    let returned;
    try {
      markStart(tagName);
      returned = ret.el.tag.call(ctx.ctx, ret.el.props, ctx.ctx);
    } catch (err) {
      setFlag2(ctx.ret, IsErrored2);
      throw err;
    } finally {
      setFlag2(ctx.ret, IsExecuting2, false);
    }
    if (isIteratorLike2(returned)) {
      ctx.iterator = returned;
    } else if (!isPromiseLike2(returned)) {
      measureMark(tagName);
      return [
        void 0,
        diffComponentChildren2(ctx, returned, false)
      ];
    } else {
      const returned1 = returned instanceof Promise ? returned : Promise.resolve(returned);
      returned1.then(() => measureMark(tagName), () => measureMark(tagName));
      return [
        returned1.catch(NOOP2),
        returned1.then((returned2) => diffComponentChildren2(ctx, returned2, false), (err) => {
          setFlag2(ctx.ret, IsErrored2);
          throw err;
        })
      ];
  ...
  if (initial) {
    ...
    if (isPromiseLike2(iteration)) {
      setFlag2(ctx.ret, IsAsyncGen2);
    } else {
      setFlag2(ctx.ret, IsSyncGen2);
    }
  }
  if (getFlag2(ctx.ret, IsSyncGen2)) {
    if (!initial) {
      try {
        setFlag2(ctx.ret, IsExecuting2);
        const oldResult = ctx.adapter.read(getValue2(ctx.ret));
        markStart(tagName);
        iteration = ctx.iterator.next(oldResult);
        measureMark(tagName);
      } catch (err) {
    ...
    if (getFlag2(ctx.ret, IsInForOfLoop2) && !getFlag2(ctx.ret, NeedsToYield2) && !getFlag2(ctx.ret, IsUnmounted2) && !getFlag2(ctx.ret, IsSchedulingRefresh2)) {
      console.error(`Component <${getTagName2(ctx.ret.el.tag)}> yielded/returned more than once in for...of loop`);
    }
    setFlag2(ctx.ret, NeedsToYield2, false);
    setFlag2(ctx.ret, IsSchedulingRefresh2, false);
    if (iteration.done) {
      setFlag2(ctx.ret, IsSyncGen2, false);
      ctx.iterator = void 0;
    }
    const diff2 = diffComponentChildren2(ctx, iteration.value, !iteration.done);
    const block = isPromiseLike2(diff2) ? diff2.catch(NOOP2) : void 0;
    return [block, diff2];
  } else {
    if (getFlag2(ctx.ret, IsInForAwaitOfLoop2)) {
      measureMark(tagName);
      pullComponent2(ctx, iteration);
      const block = resumePropsAsyncIterator2(ctx);
      return [block, ctx.pull && ctx.pull.diff];
    } else {
      resumePropsAsyncIterator2(ctx);
      if (!initial) {
        try {
          setFlag2(ctx.ret, IsExecuting2);
          const oldResult = ctx.adapter.read(getValue2(ctx.ret));
      ...
      const diff2 = iteration.then((iteration2) => {
        ...
        setFlag2(ctx.ret, IsSchedulingRefresh2, false);
        if (iteration2.done) {
          setFlag2(ctx.ret, IsAsyncGen2, false);
          ctx.iterator = void 0;
        }
        return diffComponentChildren2(
          ctx,
          // Children can be void so we eliminate that here
          iteration2.value,
          !iteration2.done
      ...
      return [diff2.catch(NOOP2), diff2];
    }
  }
}
function resumePropsAsyncIterator2(ctx) {
  if (ctx.onPropsProvided) {
    ctx.onPropsProvided(ctx.ret.el.props);
    ctx.onPropsProvided = void 0;
    setFlag2(ctx.ret, PropsAvailable2, false);
  } else {
    setFlag2(ctx.ret, PropsAvailable2);
    if (getFlag2(ctx.ret, IsInForAwaitOfLoop2)) {
      return new Promise((resolve) => ctx.onPropsRequested = resolve);
    }
  }
  return ctx.pull && ctx.pull.iterationP && ctx.pull.iterationP.then(NOOP2, NOOP2);
}
async function pullComponent2(ctx, iterationP) {
  if (!iterationP || ctx.pull) {
    return;
  }
  ctx.pull = { iterationP: void 0, diff: void 0, onChildError: void 0 };
  ...
  try {
    let childError;
    while (!done) {
      if (isPromiseLike2(iterationP)) {
        ctx.pull.iterationP = iterationP;
      }
      let onDiff;
      ctx.pull.diff = new Promise((resolve) => onDiff = resolve).then(() => {
        if (!(getFlag2(ctx.ret, IsUpdating2) || getFlag2(ctx.ret, IsRefreshing2))) {
          commitComponent2(ctx, []);
        }
      }, (err) => {
        if (!(getFlag2(ctx.ret, IsUpdating2) || getFlag2(ctx.ret, IsRefreshing2)) || // TODO: is this flag necessary?
        !getFlag2(ctx.ret, NeedsToYield2)) {
          return propagateError2(ctx, err, []);
        }
        throw err;
      });
      let iteration;
      ...
      try {
        if (!isPromiseLike2(iterationP)) {
          diff2 = void 0;
        } else if (!getFlag2(ctx.ret, NeedsToYield2) && getFlag2(ctx.ret, PropsAvailable2) && getFlag2(ctx.ret, IsInForAwaitOfLoop2)) {
          diff2 = void 0;
        } else {
          diff2 = diffComponentChildren2(ctx, iteration.value, !iteration.done);
        }
      } catch (err) {
        onDiff(Promise.reject(err));
      } finally {
      ...
      if (getFlag2(ctx.ret, IsUnmounted2)) {
        while ((!iteration || !iteration.done) && ctx.iterator && getFlag2(ctx.ret, IsInForAwaitOfLoop2)) {
          try {
            setFlag2(ctx.ret, IsExecuting2);
            iteration = await ctx.iterator.next(oldResult);
          } catch (err) {
        ...
        break;
      } else if (!getFlag2(ctx.ret, IsInForAwaitOfLoop2)) {
        break;
      } else if (!iteration.done) {
        try {
          setFlag2(ctx.ret, IsExecuting2);
...
function commitComponent2(ctx, schedulePromises, hydrationNodes) {
  if (ctx.schedule) {
    ctx.schedule.promise.then(() => {
      commitComponent2(ctx, []);
      propagateComponent2(ctx);
    });
    return getValue2(ctx.ret);
  }
  const values = commitChildren2(ctx.adapter, ctx.host, ctx, ctx.scope, ctx.root, ctx.ret, ctx.index, schedulePromises, hydrationNodes);
  if (getFlag2(ctx.ret, IsUnmounted2)) {
    return;
  }
  addEventTargetDelegates2(ctx.ctx, values);
  const wasScheduling = getFlag2(ctx.ret, IsScheduling2);
  let schedulePromises1;
  const callbacks = scheduleMap2.get(ctx);
  if (callbacks) {
    scheduleMap2.delete(ctx);
    setFlag2(ctx.ret, IsScheduling2);
    const result = ctx.adapter.read(unwrap2(values));
    for (const callback of callbacks) {
      const scheduleResult = callback(result);
      if (isPromiseLike2(scheduleResult)) {
        (schedulePromises1 = schedulePromises1 || []).push(scheduleResult);
      }
    }
    if (schedulePromises1 && !getFlag2(ctx.ret, DidCommit2)) {
      const scheduleCallbacksP = Promise.all(schedulePromises1).then(() => {
        setFlag2(ctx.ret, IsScheduling2, wasScheduling);
        propagateComponent2(ctx);
        if (ctx.ret.fallback) {
          unmount2(ctx.adapter, ctx.host, ctx.parent, ctx.root, ctx.ret.fallback, false);
        }
        ctx.ret.fallback = void 0;
      });
      let onAbort;
      const scheduleP = safeRace2([
        scheduleCallbacksP,
        new Promise((resolve) => onAbort = resolve)
      ]).finally(() => {
        ctx.schedule = void 0;
  ...
  if (!getFlag2(ctx.ret, IsScheduling2)) {
    if (!getFlag2(ctx.ret, IsUpdating2)) {
      propagateComponent2(ctx);
    }
    if (ctx.ret.fallback) {
      unmount2(ctx.adapter, ctx.host, ctx.parent, ctx.root, ctx.ret.fallback, false);
    ...
    ctx.ret.fallback = void 0;
    setFlag2(ctx.ret, IsUpdating2, false);
  }
  setFlag2(ctx.ret, DidCommit2);
  return getValue2(ctx.ret, true);
}
function isRetainerActive2(target, host) {
  const stack = [host];
  while (stack.length > 0) {
    const current = stack.pop();
    if (current === target) {
    ...
    const isHostBoundary = current !== host && (typeof current.el.tag === "string" && current.el.tag !== Fragment2 || current.el.tag === Portal2);
    if (current.children && !isHostBoundary) {
      if (Array.isArray(current.children)) {
        for (const child of current.children) {
          if (child) {
    ...
    if (current.fallback && !getFlag2(current, DidDiff2)) {
      stack.push(current.fallback);
    }
  }
  return false;
}
function propagateComponent2(ctx) {
  const values = getChildValues2(ctx.ret, ctx.index);
  addEventTargetDelegates2(ctx.ctx, values, (ctx1) => ctx1[_ContextState2].host === ctx.host);
  const host = ctx.host;
  const initiator = ctx.ret;
  if (!isRetainerActive2(initiator, host)) {
    return;
  }
  if (!getFlag2(host, DidCommit2)) {
    return;
  }
  const props = stripSpecialProps2(host.el.props);
  const hostChildren = getChildValues2(host, 0);
  ctx.adapter.arrange({
    tag: host.el.tag,
    tagName: getTagName2(host.el.tag),
    node: host.value,
    props,
    oldProps: props,
    children: hostChildren,
...
async function unmountComponent2(ctx, isNested) {
  if (getFlag2(ctx.ret, IsUnmounted2)) {
    return;
  }
  let cleanupPromises;
  const callbacks = cleanupMap2.get(ctx);
  if (callbacks) {
    const oldResult = ctx.adapter.read(getValue2(ctx.ret));
    cleanupMap2.delete(ctx);
    for (const callback of callbacks) {
      const cleanup = callback(oldResult);
  ...
  setFlag2(ctx.ret, IsUnmounted2);
  if (ctx.schedule) {
    ctx.schedule.onAbort();
    ctx.schedule = void 0;
  }
  clearEventListeners2(ctx.ctx);
  unmountChildren2(ctx.adapter, ctx.host, ctx, ctx.root, ctx.ret, isNested);
  if (didLinger) {
    if (ctx.root != null) {
      ctx.adapter.finalize(ctx.root);
    }
  ...
  if (ctx.iterator) {
    ...
    let iteration;
    if (getFlag2(ctx.ret, IsInForOfLoop2)) {
      try {
        setFlag2(ctx.ret, IsExecuting2);
        const oldResult = ctx.adapter.read(getValue2(ctx.ret));
        const iterationP = ctx.iterator.next(oldResult);
        if (isPromiseLike2(iterationP)) {
          if (!getFlag2(ctx.ret, IsAsyncGen2)) {
            throw new Error("Mixed generator component");
          }
          iteration = await iterationP;
        } else {
...
function handleChildError2(ctx, err) {
  if (!ctx.iterator) {
    throw err;
  }
  if (ctx.pull) {
  ...
  if (isPromiseLike2(iteration)) {
    return iteration.then((iteration2) => {
      ...
      return diffComponentChildren2(ctx, iteration2.value, !iteration2.done);
    }, (err2) => {
      setFlag2(ctx.ret, IsErrored2);
      throw err2;
    });
  ...
  return diffComponentChildren2(ctx, iteration.value, !iteration.done);
}
function propagateError2(ctx, err, schedulePromises) {
  const parent = ctx.parent;
  if (!parent) {
    throw err;
  }
  let diff2;
  try {
    diff2 = handleChildError2(parent, err);
  } catch (err2) {
    return propagateError2(parent, err2, schedulePromises);
  }
  if (isPromiseLike2(diff2)) {
    return diff2.then(() => void commitComponent2(parent, schedulePromises), (err2) => propagateError2(parent, err2, schedulePromises));
  }
  commitComponent2(parent, schedulePromises);
}
...
function* Editable({ state, children }) {
  ...
  for ({ state, children } of this) {
    ...
    const areaEl = yield createElement2(
      "content-area",
      null,
      children
    );
...
function* SimpleEditable({ initial }) {
  ...
  for ({} of this) {
    ...
    let cursor = 0;
    yield /* @__PURE__ */ createElement(Editable, { state, onstatechange: () => this.refresh() }, /* @__PURE__ */ createElement("pre", { class: "editable", contenteditable: "true", spellcheck: false }, lines.map((line) => {
...
function* RainbowEditable({ initial }) {
  ...
  for ({} of this) {
    ...
    yield /* @__PURE__ */ createElement(Editable, { state, onstatechange: () => this.refresh() }, /* @__PURE__ */ createElement("div", { class: "editable", contenteditable: "true", spellcheck: false, hydrate: "!children" }, lines.map((line) => {
      const key = state.keyer.keyAt(cursor);
      cursor += line.length + 1;
...
function* CodeEditable({ initial }) {
  ...
  for ({} of this) {
    ...
    let cursor = 0;
    yield /* @__PURE__ */ createElement(Editable, { state, onstatechange: () => this.refresh() }, /* @__PURE__ */ createElement("pre", { class: "editable", contenteditable: "true", spellcheck: false, hydrate: "!children" }, lines.map((line) => {
      const key = state.keyer.keyAt(cursor);
...
function* BlockquoteEditable({ initial }) {
  ...
  for ({} of this) {
    ...
    let cursor = 0;
    yield /* @__PURE__ */ createElement(Editable, { state, onstatechange: () => this.refresh() }, /* @__PURE__ */ createElement(
      "div",
      {
        class: "editable",
    ...
    yield /* @__PURE__ */ createElement(Editable, { state, onstatechange: () => this.refresh() }, /* @__PURE__ */ createElement(
      ...
      lines.map((line) => {
        ...
        if (match) {
          ...
          ), /* @__PURE__ */ createElement("span", { style: checked ? { textDecoration: "line-through", opacity: "0.5" } : void 0 }, match[2] || /* @__PURE__ */ createElement("br", null)));
        }
        return /* @__PURE__ */ createElement("div", { key }, line || /* @__PURE__ */ createElement("br", null));
      })
    ));
