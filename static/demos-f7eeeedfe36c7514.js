...
function getRootRetainer(renderer2, bridge, { children, root, hydrate }) {
  let ret;
...
var supportsUserTiming2 = typeof performance !== "undefined" && typeof performance.mark === "function";
function markStart2(label) {
  if (supportsUserTiming2) {
    performance.mark("\u2699 " + label);
  }
}
function measureMark2(label) {
  if (supportsUserTiming2) {
    const name = "\u2699 " + label;
    try {
      performance.measure(name, name);
...
function runComponent2(ctx) {
  ...
  if (initial) {
    ...
    if (isIteratorLike2(returned)) {
      ...
      const returned1 = returned instanceof Promise ? returned : Promise.resolve(returned);
      returned1.then(() => measureMark2(tagName), () => measureMark2(tagName));
      return [
  ...
  if (getFlag2(ctx.ret, IsSyncGen2)) {
    ...
    if (getFlag2(ctx.ret, IsInForAwaitOfLoop2)) {
      ...
      iteration.then(() => measureMark2(tagName), () => measureMark2(tagName));
      const diff2 = iteration.then((iteration2) => {
        if (getFlag2(ctx.ret, IsInForAwaitOfLoop2)) {
...
if (titleEl) {
  titleEl.innerHTML = "";
  renderer.render(
    /* @__PURE__ */ createElement(EditableTitle, { initial: titleEl.dataset.initial || "\n" }),
    titleEl
  );
}
...
if (taglineEl) {
  taglineEl.innerHTML = "";
  renderer.render(
    /* @__PURE__ */ createElement(EditableTagline, { initial: taglineEl.dataset.initial || "\n" }),
    taglineEl
  );
}
