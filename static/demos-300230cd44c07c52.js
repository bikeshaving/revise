    ...
    for (let i = 0; i < result.parts.length - 1; i += 3) {
      ...
      if (deleted2 && inserted2) {
        ...
        if (prefixLen > 0 || suffixLen > 0) {
          const optimizedDeleted = deleted2.slice(prefixLen, deleted2.length - suffixLen);
          const optimizedInserted = inserted2.slice(prefixLen, inserted2.length - suffixLen);
...
function synthesize(insertSeq, inserted, deleteSeq, deleted) {
  ...
  for (const [length, deleting, inserting] of align(expandedDeleteSeq, insertSeq)) {
...
function validateEditParts(parts) {
  ...
  if (parts.length !== 1 && (parts.length - 1) % 3 !== 0) {
    throw new Error(`Edit parts length ${parts.length} is invalid - must be 1 or (operations * 3 + 1)`);
  }
  if (parts.length === 1) {
  ...
  for (let i = 0; i < parts.length - 1; i += 3) {
    ...
    if (typeof position !== "number") {
      throw new Error(`Position at index ${i} must be a number, got ${typeof position}`);
    }
    if (typeof deleted !== "string") {
      throw new Error(`Deleted at index ${i + 1} must be a string, got ${typeof deleted}`);
    }
    if (typeof inserted !== "string") {
      throw new Error(`Inserted at index ${i + 2} must be a string, got ${typeof inserted}`);
    }
    if (position < 0) {
      throw new Error(`Position ${position} at index ${i} cannot be negative`);
    }
    if (position <= previousPos) {
      throw new Error(`Position ${position} at index ${i} must be > previous end position ${previousPos}`);
    }
    ...
    if (nextIndex < parts.length - 1) {
      ...
      if (deletionEnd > nextPos) {
        throw new Error(`Deletion at position ${position} extends to ${deletionEnd}, exceeding next position ${nextPos}`);
      }
    } else {
      if (deletionEnd > finalPos) {
        throw new Error(`Deletion at position ${position} extends to ${deletionEnd}, exceeding final position ${finalPos}`);
      }
    }
    previousPos = deletionEnd;
  }
  if (previousPos > finalPos) {
    throw new Error(`Operations extend to position ${previousPos}, exceeding final position ${finalPos}`);
  }
}
...
var EditableState = class extends EventTarget {
  value;
  history;
  keyer;
  selection;
...
// src/components/crank-editable.tsx
init_Buffer();
init_process();
function* CrankEditable({ state, children }) {
  ...
  this.addEventListener("keydown", (ev) => {
    ...
    if (ev.key === "z" || ev.key === "Z") {
      ev.preventDefault();
      handled = ev.shiftKey ? state.redo() : state.undo();
  ...
  for ({ state, children } of this) {
    ...
    const areaEl = yield /* @__PURE__ */ createElement("content-area", null, children);
...
function* RainbowEditable({ initial }) {
  ...
  for (const {} of this) {
    ...
    yield /* @__PURE__ */ createElement(CrankEditable, { state, onstatechange: () => this.refresh() }, /* @__PURE__ */ createElement("div", { class: "editable", contenteditable: "true", spellcheck: false }, lines.map((line) => {
      const key = state.keyer.keyAt(cursor);
      cursor += line.length + 1;
...
function* CodeEditable({ initial }) {
  ...
  for (const {} of this) {
    const lines = splitLines(
      import_prismjs.default.tokenize(state.value, import_prismjs.default.languages.typescript)
    );
    let cursor = 0;
    yield /* @__PURE__ */ createElement(CrankEditable, { state, onstatechange: () => this.refresh() }, /* @__PURE__ */ createElement("pre", { class: "editable", contenteditable: "true", spellcheck: false }, lines.map((line) => {
      const key = state.keyer.keyAt(cursor);
...
function highlightSocial(text) {
  ...
  for (const match of text.matchAll(SOCIAL_PATTERN)) {
    ...
    result.push(/* @__PURE__ */ createElement("span", { style: { color } }, value));
