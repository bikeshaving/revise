...
export function appendSegment(segments: Array<number>, length: number, flag: boolean): void {
  if (length < 0) {
...
export class Subseq1 {
	...
	union(that: Subseq1): Subseq1 {
		const segments: Array<number> = [];
		this.align(that, (l, f1, f2) => appendSegment(segments, l, f1 || f2));
		return new Subseq1(segments);
	}
	...
	expand(that: Subseq1, options: {fill?: boolean} = {}): Subseq1 {
		...
		for (
			...
			f2 = !f2;
			if (f2) {
				appendSegment(segments, l2, !!options.fill);
			} else {
				while (l2) {
					if (l1 === 0) {
						if (i1 >= this.segments.length) {
