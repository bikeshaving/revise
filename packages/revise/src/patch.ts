 ...
 * copy or retain operation, where the numbers represent the start-inclusive and
 * end-exclusive range which should be copied over to the result. Deletions are
 * represented via omission, i.e. a gap between two copy operations. Strings
 * within a patch represent insertions at the current index. A -1 before a
 * string indicates the string is added and immediately removed. This is useful
 * for representing squashed patches or patches made against a snapshot. The
 * last element of a patch will always be a number which represents the length
 * of the text before modification.
 */
export type Patch = Array<number | string>;
...
function append(patch: Patch, op: string | number): number {
...
export function synthesize(
	...
	for (const [length, iFlag, dFlag] of zip(insertSeq, deleteSeq)) {
		if (iFlag) {
			...
			if (!dFlag || prevIFlag) {
				append(patch, retainIndex + length);
			}
	...
	get operations(): Array<Operation> {
		...
		for (let i = 0; i < this.parts.length; i++) {
			...
			if (typeof part === "number") {
				...
				result.push({type: "insert", start: insertOffset, inserted: part});
				insertOffset = retainOffset;
				retaining = true;
			} else {
