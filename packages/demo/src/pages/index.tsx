...
function Editor() {
	...
	React.useEffect(() => {
		...
		CollabText.initialize("doc1", client).then(async (text) => {
			...
			handleChange = (cm, change) => {
				...
				console.log("SETTING VERSION IN HANDLE CHANGE", version);
			};
			...
			for await (const _ of text.subscribe()) {
				...
				version = { commit: update.commit, change: update.change };
				console.log("SETTING VERSION IN SUBSCRIBE", version);
			}
		});
		return () => {
			unmounted = true;
	...
	return (
		<div
			ref={editor}
			style={{ color: 'white', backgroundColor: 'black', margin: "0 auto", padding: 20, maxWidth: 600 }}
		/>
	);
}
