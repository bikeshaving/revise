...
function transformJSXAttributes(attributes: any[]): {
	...
	for (const attr of attributes) {
		if (attr.type === "JSXSpreadAttribute") {
			...
			// Use a string literal key for names with hyphens (e.g. data-content)
			// since they're not valid JS identifiers
			const key: ESTree.Expression = /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(name)
...
/**
 * Rewrite bare module specifiers to use jsdelivr CDN.
...
function rewriteBareModuleSpecifiers(ast: any): void {
	...
	function rewrite(value: string): string {
		...
		// Skip @b9g/* imports - they're handled by the import map
		if (
			value.startsWith("@b9g/crank") ||
			value.startsWith("@b9g/revise") ||
			value.startsWith("@b9g/crankeditable")
		) {
			return value;
		}
